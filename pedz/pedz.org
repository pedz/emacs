...
#+BEGIN_COMMENT
I'm switching from projectile to project (built into emacs) and so I
have stopped loading and using projectile-setup.org, ruby-setup.org,
and project-setups.org.  When I start working on Hatred again, I will
get all that working again.  These used lsp-mode and projectile and I
am moving to eglot and project because lsp-mode didn't seem to work on
...
#+begin_src emacs-lisp
  ...
  (define-key pedz/personal-map (kbd "l")   #'list-matching-lines)
...
I am using swift-ts-mode instead of swift-mode so this is here but
turned off.  Both have been assimilated so it is just a matter of
which is hooked to trigger when a Swift file is loaded.
...
To get [[https://github.com/rechsteiner/swift-ts-mode][swift-ts-mode]] to work, the [[https://github.com/alex-pinkus/tree-sitter-swift][Swift tree-sitter grammar]] needs to
be installed.

    1. Go to the [[https://github.com/alex-pinkus/tree-sitter-swift/actions][GitHub actions page]].
    2. Click on the "Publish grammar.json and parser.c".
    3. Click on the topmost "run" in the list with the green
       checkmarks.
    4. Scroll down to Artifacts and click on "generated-parser-src".
       This will download a zip file.
    5. Unzip the file and cd into the directory.
    6. Compile parser.c and scanner.c with:\\
       ~cc -fPIC -c -I. parser.c~\\
       ~cc -fPIC -c -I. scanner.c~
    7. Then link the two objects with:\\
       ~cc -fPIC -shared *.o -o libtree-sitter-swift.dylib~
    8. Move =libtree-sitter-swift.dylib= into
...
I don't know where I got the value for org-html-head but the kbd
results in a very nice keyboard display.  And the dt, dd, and dl tags
result in attractive description lists.
...
*** Org-roam
Org-roam I hope will help me with all my notes and keep track of the
time I spend on various projects.
...
This is still new.  As I'm writing this, I got eglot to work last
night and today I'm switching from projectile to project and trying to
verify tht eglot is working.  I hope to circle back here and clean
things up when I'm confident they are working.

#+begin_src emacs-lisp
  (declare-function pedz/org-require "early-init")

  ;;; Currently eglot mode is used only for Swift
  (leaf eglot
...
I was using lsp-mode and projectile but I am switching to eglot and
project.

[[emacs-lsp/lsp-mode]] is the piece inside Emacs that talks to
...
#+begin_src emacs-lisp
  ...
  ;;; swift-mode and swift-ts-mode do NOT use lsp-mode but use eglot mode instead.
  (leaf lsp-mode
    :commands lsp
    ; left as a reminder.
    ; :hook (swift-mode-hook . lsp)
    ; :hook (swift-ts-mode-hook . lsp)
  ...
  ;;; I assume this will get pulled in via some autoload
  (leaf leaf-ui)

  ;;; Again, I assume this will get pulled in via an autoload.
  (leaf lsp-sourcekit)
...
UPDATE 2026-04-12 -- I removed platformio-mode because I had local
changes that I didn't understand and could not find any reference to
them.  If and when I start doing embedded development again, I will
need to figure out what I did and why.

PlatformIO is used for embedded projects such as those using the
RP2040, Arduino, etc.

Aside from the comment below, when I went to update my Emacs startup I
found that platformio-mode had changes that I could not track down as
to who made them.  I vaguely remember issues with platformio-mode but
I can't find any trace of what it was or how it was fixed.  So, I am
going to just delete it from the startup and re-figure it out if and
when I start doing embedded projects again.
...
#+begin_src emacs-lisp :tangle no
  (use-package project
    :config
    (add-hook 'project-find-functions #'platformio-project-root-override))
#+end_src
