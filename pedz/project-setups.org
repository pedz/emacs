...
These are new concepts to me so I'm going to explore the details of
how they work.  I'm going to start with my Hatred project first.
Utopia would be to have the classes be things like /rails project
using docker/ but I've learned it is easier to just do code for the
specific situation first and then morph the result into more generic
concepts as other examples are encountered.
...
#+begin_src emacs-lisp
  ...
  ;;; I *think* the reason I set variables this way is because I think it adds
  ;;; a little bit of security.  For example, I can set
  ;;; org-confirm-babel-evaluate to nil which which is unsafe but it does not
  ;;; make an entry that it is always safe so if the variable/value pair shows
  ;;; up some other place, Emacs will ask me to confirm it is ok.
...
This is a project of providing supplemental documentation to Canon EOS
cameras.  It is mostly an Org project.

#+begin_src emacs-lisp
  (defun pedz/canon-eos-doc-org-mode-locals ()
    "Sets the local variables and modes for the Org mode variables in the Canon
         EOS Documentation project"
    (message "pedz/canon-eos-doc-org-mode-locals called")
    (pedz/set-one-apply
     '(
       (org-html-head . "<style>
  img.print_icon {
  ...
  ")
      ...
      (dir-locals-set-class-variables
       'canon-eos-doc-variables
       '((org-mode . ((eval . (pedz/canon-eos-doc-org-mode-locals))))))

      ;;; and is reference here.
      (dir-locals-set-directory-class "~/Source/Canon-EOS-Documentation-Project" 'canon-eos-doc-variables)
